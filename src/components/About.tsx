...
function Card({
  ...
  className?: string;
  glowColor?: 'purple' | 'pink' | 'green' | 'blue';
}) {
  const glows = {
  ...
  return (
    <div
      className={`group relative rounded-2xl bg-white/10 backdrop-blur-md border border-white/[0.06] overflow-hidden transition-all duration-500 ${glows[glowColor]} ${className}`}
    >
      {/* inner top glow */}
      <div
        className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-br ${glows[glowColor].split(' ')[1]} to-transparent pointer-events-none`}
      />
        ...
        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-3 gap-4 auto-rows-auto">
          ...
          <Card className="md:col-span-2 p-7" glowColor="purple">
            ...
            <p className="text-gray-300 leading-relaxed text-sm mb-4">
              ...
              <span className="text-purple-300 font-medium">full-stack development</span> and{' '}
              <span className="text-pink-300 font-medium">cyber security</span>.
            </p>
            ...
            <div className="flex flex-wrap gap-2 mt-5">
              {['React', 'TypeScript', 'Node.js', 'FastAPI', 'Penetration Testing', 'AI/ML'].map(
                (t) => (
                  <span
                    key={t}
                    className="px-2.5 py-1 rounded-md text-[11px] font-medium text-purple-300 bg-purple-500/10 border border-purple-500/20"
                  >
                    {t}
                  </span>
                ),
          ...
          <Card className="p-7" glowColor="pink">
            <p className="text-xs text-pink-400 font-semibold uppercase tracking-widest mb-5">
              By the numbers
            </p>
